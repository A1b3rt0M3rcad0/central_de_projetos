...
async def find_status(status_id:int, request:Request):
    try:
...
@routes.post('/')
async def create_status(body:CreateStatusFormat, request:Request):
    try:
...
@routes.delete('/')
async def delete_status(body:DeleteStatusFormat, request:Request):
    try:
...
@routes.patch('/')
async def update_status_description(body:UpdateStatusDescriptionFormat, request:Request):
    try:
...
@routes.get('/all/')
async def find_all_status(request:Request):
    try:
...
async def find_project_by_status(status_id:int, request:Request):
    try:
        request.path_params['status_id'] = status_id
