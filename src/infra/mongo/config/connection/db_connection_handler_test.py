...
def mock_string_connection():
    return MockStringConnection('mongodb://localhost:27017')
...
def test_init_connection_handler(mock_env_vars, mock_string_connection):
...
def test_connect_to_db(mock_mongo_client_class, mock_env_vars, mock_string_connection):
...
@patch('src.infra.mongo.config.connection.db_connection_handler.MongoClient')
def test_context_manager(mock_mongo_client_class, mock_env_vars, mock_string_connection):
    mock_client = MagicMock()
    mock_db = MagicMock()
    mock_client.__getitem__.return_value = mock_db
    mock_mongo_client_class.return_value = mock_client
